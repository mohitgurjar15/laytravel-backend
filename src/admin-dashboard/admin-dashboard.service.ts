...
export class AdminDashboardService {
    ...
    async bookingStatistics(filterOption: DashboardFilterDto) {
      ...
      FROM "booking" WHERE date(check_in_date) < date('${todayDate}') AND booking_status = ${BookingStatus.CONFIRM} AND ${moduleIdCondition} AND ${dateConditon}`);
            ...
            response["uncovered_Pre_Bookings"] =
                uncoveredPreBookings[0].cnt || 0;
            ...
            response["uncovered_Pre_Bookings_Usd"] =
                (
                    Math.round(uncoveredPreBookingsUsd[0].total_amount * 100) /
                    100
                ).toFixed(2) || 0;
      ...
      from booking where booking_type = ${BookingType.NOINSTALMENT} AND booking_status = ${BookingStatus.CONFIRM} AND ${moduleIdCondition} AND ${dateConditon}`);
            ...
            var revenueFromNoInstallmentPercent =
                (revenueFromNoInstallment * 100) / revenues[0].total_profit ||
                0;
            response["revenue_from_full_price_percentage"] =
                (
                    Math.round(revenueFromNoInstallmentPercent * 100) / 100
                ).toFixed(2) || 0;
      ...
      from booking where  booking_type = ${BookingType.INSTALMENT} AND ${moduleIdCondition} AND ${dateConditon}`);
            response["booking_with_installment_qty"] =
                installmentBookingQty[0].confirm_booking || 0;
      ...
      from booking where booking_type = ${BookingType.NOINSTALMENT}  AND ${moduleIdCondition} AND ${dateConditon}`);
            response["booking_with_no_installment_qty"] =
                noInstallmentBookingQty[0].confirm_booking || 0;

            var valueOfBooking = await getConnection().query(`
      SELECT  SUM( total_amount / usd_factor) as total_amount from booking where 1=1 AND (("booking"."booking_status" = 0 AND "booking"."payment_status" = 0)or("booking"."booking_status" = 1 AND "booking"."payment_status" = 0 )or("booking"."payment_status" = 1))AND ${moduleIdCondition} AND ${dateConditon}
`);

            response["value_of_bookings"] = valueOfBooking[0].total_amount || 0;

            var valueOfBookingQty = await getConnection().query(`
      SELECT  count(id) as cnt from booking where 1=1 AND (("booking"."booking_status" = 0 AND "booking"."payment_status" = 0)or("booking"."booking_status" = 1 AND "booking"."payment_status" = 0 )or("booking"."payment_status" = 1))AND ${moduleIdCondition} AND ${dateConditon}
      `);
      ...
      select sum(amount) as total_amount from booking_instalments where payment_status = ${PaymentStatus.CONFIRM}  
            ...
            response["fully_paid_by_laytrip"] =
                (Math.round(FullyPaidByLayTrip[0].total * 100) / 100).toFixed(
                    2
                ) || 0;
            ...
            response["customer_credit_at_risk"] =
                (Math.round(custCredAtRisk[0].total * 100) / 100).toFixed(2) ||
                0;
        ...
        AND ${moduleIdCondition} AND ${dateConditon}`);
            response["gross_sales"] =
                (Math.round(grossSales[0].total_amount * 100) / 100).toFixed(
                    2
                ) || 0;
    ...
    SELECT
      ...
      ${userConditon}`);
        ...
        const grossPerUser =
            parseFloat(sumOfTotalAmount[0].total_amount) /
            parseFloat(countOfUserEnum[0].user_count);
        response["gross_per_user"] = (
            Math.round(grossPerUser * 100) / 100
        ).toFixed(2);
    ...
    FROM booking `);
        ...
        const revenuePerUser =
            parseFloat(sumOfMarkUpAmount[0].total_amount) /
            parseFloat(countOfUserEnum[0].user_count);
        response["revenue_per_user"] = (
            Math.round(revenuePerUser * 100) / 100
        ).toFixed(2);

        const revenueGrowthUsd = parseFloat(sumOfMarkUpAmount[0].total_amount);
        response["revenue_growth_in_usd"] = (
            Math.round(revenueGrowthUsd * 100) / 100
        ).toFixed(2);

        const revenueGrowthPercentage =
            (100 * parseFloat(sumOfMarkUpAmount[0].total_amount)) /
            parseFloat(sumOfTotalAmount[0].total_amount);
        response["revenue_growth_percentage"] = (
            Math.round(revenueGrowthPercentage * 100) / 100
        ).toFixed(2);

        var markUpTotalBooking =
            parseFloat(sumOfMarkUpAmount[0].total_amount) /
            parseFloat(totalTicketsBooked[0].cnt);
        var totalAmtTotalBooking =
            parseFloat(sumOfTotalAmount[0].total_amount) /
            parseFloat(totalTicketsBooked[0].cnt);

        const avgBookingMarkUp =
            sumOfMarkUpAmount[0].total_amount /
            parseFloat(totalTicketsBooked[0].cnt);
        response["average_booking_markup_percentage"] = (
            Math.round(avgBookingMarkUp * 100) / 100
        ).toFixed(2);

        const avgBookingMarkUpUsd = sumOfMarkUpAmount[0].total_amount;
        response["average_booking_markup_usd"] = (
            Math.round(avgBookingMarkUpUsd * 100) / 100
        ).toFixed(2);
      ...
      GROUP BY
        ...
        response["sales_by_full_price_count"] =
            Math.round(salesByFullPriceCount.length * 100) / 100;

        const totalBooking = await getConnection().query(`
                SELECT  count(id) as cnt from booking`);

        const salesByFullPricePercent =
            (salesByFullPriceCount.length * 100) / totalBooking[0].cnt;
        response["sales_by_full_price_percent"] =
            Math.round(salesByFullPricePercent * 100) / 100;

        const salesByFullPriceRevenue = await getConnection().query(`
    SELECT sum("booking"."markup_amount"/"booking"."usd_factor") as "total_amount" 
    FROM booking WHERE booking_type In (${BookingType.INSTALMENT}) 
    `);
        response["sales_by_full_price_revenue"] =
            Math.round(salesByFullPriceRevenue[0].total_amount * 100) / 100;
      ...
      GROUP BY
        ...
        response["sales_by_installment_count"] =
            Math.round(salesByInstallmentCount.length * 100) / 100;

        const salesByInstallmentPercent =
            (salesByInstallmentCount.length * 100) / totalBooking[0].cnt;
        response["sales_by_installment_percent"] =
            Math.round(salesByInstallmentPercent * 100) / 100;

        const salesByInstallmentRevenue = await getConnection().query(`
    SELECT sum("booking"."markup_amount"/"booking"."usd_factor") as "total_amount" 
    FROM booking WHERE booking_type In (${BookingType.NOINSTALMENT}) 
    `);
        response["sales_by_installment_revenue"] =
            Math.round(salesByInstallmentRevenue[0].total_amount * 100) / 100;
