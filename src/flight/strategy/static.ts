...
export class Static implements StrategyAirline{
    ...
    async oneWaySearch(searchFlightDto:OneWaySearchFlightDto){
        ...
        let searchItem={ stop:0, search_type:'oneway', route_type:'outbound' ,  price:0.00,route_details: [],};
        
        if(result.length){

            for(let i=0; i < result.length; i++){
                    
                searchItem={ stop:0, price:0.00, route_details: [], search_type:'oneway', route_type:'outbound' };
                if(result[i].departure.id ===departureId && result[i].arrival.id===arrivalId){
                
                    searchItem.stop=0;
                    result[i]['departureDate']=departure_date; 
                    result[i]['arrivalDate']=departure_date; 
                    ...
                    if(result[i].departure.id==departureId){
                        ...
                        result.forEach((res,j)=>{
                            ...
                            if(res.departure.id === xArrivalId && (res.departure.id !==departureId) && i!=j){
                                res['departureDate']=departure_date; 
                                res['arrivalDate']=departure_date; 
    ...
    async roundTripSearch(searchFlightDto:RoundtripSearchFlightDto){
        ...
        let searchResult=[];
        let supplierData;
        let outboundResult =  await getManager()
            .createQueryBuilder(FlightRoute, "FlightRoute")
            .leftJoinAndSelect("FlightRoute.airline","airline")
            .leftJoinAndSelect("FlightRoute.arrival","arrival")
            .leftJoinAndSelect("FlightRoute.departure","departure")
        ...
        if(outboundResult.length > 0){
            ...
            outboundResult.forEach(async (res,i) => {
        ...
        let supplierId = typeof supplierData!='undefined'?supplierData.supplierId : 0
        let searchItem={ stop:0, search_type:'roundtrip', route_type:'outbound' ,  price:0.00,route_details: [],};
        
        if(outboundResult.length){

            for(let i=0; i < outboundResult.length; i++){
                    
                searchItem={ stop:0, price:0.00, route_details: [], search_type:'roundtrip', route_type:'outbound' };
                if(outboundResult[i].departure.id ===departureId && outboundResult[i].arrival.id===arrivalId){
                
                    searchItem.stop=0;
                    searchItem.route_details.push(outboundResult[i]);
                    outboundResult[i]['departureDate']=departure_date; 
                    outboundResult[i]['arrivalDate']=departure_date; 
                    ...
                    if(outboundResult[i].departure.id==departureId){
                            
                        let xArrivalId = outboundResult[i].arrival.id;
                        searchItem.route_details.push(outboundResult[i])
                        
                        outboundResult.forEach((res,j)=>{
