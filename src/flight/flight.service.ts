...
export class FlightService {
  ...
  async getSellingPrice(netRateDto: NetRateDto, user) {
    ...
    let secondarySellingPrice =
      PriceMarkup.applyMarkup(net_rate, result.secondaryMarkUpDetails) || 0;
    ...
    try {
      ...
      if (instalmentDetails) {
        ...
        for (let instalment of instalmentDetails.instalment_date) {
          ...
          bookingInstalment.attempt = i == 0 && captureCardresult ? 1 : 0;
    ...
    for await (const reservation of ticketDetails.data["a:itineraryinfo"][0][
      ...
      if (reservation != null) {
        var data = {
          ...
          departure_info:
            airports[reservation["a:departureairportlocationcode"][0]],
          ...
          arrival_info:
            airports[reservation["a:arrivalairportlocationcode"][0]],
          eticket: true,
  ...
  async bookPartialBooking(bookingId, Headers) {
    ...
    if (
      new Date(
        await this.changeDateFormat(bookingData.moduleInfo[0].departure_date)
      ) > new Date()
    ) {
