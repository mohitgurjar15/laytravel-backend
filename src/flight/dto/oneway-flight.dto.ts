import { IsEnum, IsNotEmpty, MaxLength, ValidateIf, ValidationArguments } from "class-validator";
...
export class OneWaySearchFlightDto {
    ...
    @MaxLength(3, {
        message: "Departure code should not be more then 3 characters",
    })
    @ApiProperty({
    ...
    @MaxLength(3, {
        message: "Arrival code should not be more then 3 characters",
    })
    @ApiProperty({
    ...
    @ValidateIf((o) => typeof o.request_date != "undefined")
    @IsValidDate("", {
        message: (args: ValidationArguments) => {
            if (typeof args.value == "undefined" || args.value == "") {

            } else {
                return `Please enter valid request date.(YYYY-MM-DD)&&&dob`;
            }
        },
    })
    @ApiProperty({
        description: `Enter your request date.`,
        example: `1995-06-22`,
    })
    request_date?: string;
}