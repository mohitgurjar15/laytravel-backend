...
export class CartService {
    ...
    try {
      ...
      if (result >= 5) {
        throw new BadRequestException(
          `In your cart you have add maximum 5 item.`
        );
      }
  ...
  async addFlightDataInCart(route_code: string, user: User, Header) {
    ...
    if (flightInfo) {
      ...
      cart.expiryDate =
        diffrence > 2 ? new Date(dayAfterDay) : new Date(formatedDepatureDate);
  ...
  async addHomeRentalDataInCart(dto, user, Header) {
    ...
    if (homeInfo) {
      ...
      cart.expiryDate =
        diffrence > 2 ? new Date(dayAfterDay) : new Date(formatedCheckinDate);
      cart.isDeleted = false;
      cart.createdDate = new Date();
