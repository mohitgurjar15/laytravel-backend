...
export class BookingService {
  ...
  async exportUpcomingPaymentForAdmin(
    ...
    const { data, total_count } = await this.bookingRepository.exportPayment(
      where
    );
    //const result: any = data;
    ...
    try {
      ...
      for await (const data of result.data) {
        ...
        predictiveBookingData["paid_amount_in_percentage"] =
          (totalPaidAmount * 100) / parseFloat(bookingData.totalAmount);
        ...
        if (net_rate_percentage_variation == 0) {
          predictiveBookingData["net_rate_percentage_variation_stage"] =
            "EQUAL";
        ...
        if (
          ...
          predictiveBookingData["net_rate_percentage_variation_stage"] == "DOWN"
        ) {
          ...
          predictiveBookingData["net_rate_percentage_variation_stage"] == "UP"
        ) {
      ...
      for await (const booking of allBooking) {
        if (availableBookingId.indexOf(booking.laytripBookingId) != -1) {
          ...
          responce.push(
            todayPrice[availableBookingId.indexOf(booking.laytripBookingId)]
          );
        } else {
          ...
          predictiveBookingData["paid_amount_in_percentage"] =
            (totalPaidAmount * 100) / parseFloat(booking.totalAmount);
  ...
  async deleteBooking(deleteBookingDto: DeleteBookingDto, user: User) {
    ...
    for await (const booking of query.bookings) {
      await getConnection()
        ...
        .where(`id =:id AND payment_status = ${PaymentStatus.PENDING}`, {
          id: booking.id,
        })
        .execute();
