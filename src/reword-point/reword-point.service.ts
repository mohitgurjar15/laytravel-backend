...
export class RewordPointService {
  ...
  async listEarnReword(
    paginationOption: ListEarnRewordDto,
    id: string
  ): Promise<{ data: LayCreditEarn[]; TotalResult: number }> {
    try {
      return await this.rewordPointEarnRepository.listEarnReword(
        paginationOption,
        id
      );
    } catch (error) {
  ...
  async listRedeemReword(
    paginationOption: ListRedeemRewordDto,
    id: string
  ): Promise<{ data: LayCreditRedeem[]; TotalResult: number }> {
    try {
      return await this.rewordPointRedeemRepository.listRedeemReword(
        paginationOption,
        id
      );
    } catch (error) {
  ...
  async countOfRewordPoints(userId: string) {
    try {
      let [earnedReword] = await this.rewordPointEarnRepository.query(
        `SELECT sum("points") FROM "lay_credit_earn" WHERE user_id = '${userId}' AND status = 1`
      );

      let [redeemReword] = await this.rewordPointRedeemRepository.query(
        `SELECT sum("points") FROM "lay_credit_redeem" WHERE user_id = '${userId}' AND status = 1`
      );
      ...
      if (payment.paymentStatus == PaymentStatus.CONFIRM) {
        if (type != LaytripPointsType.RECURRING) {
          ...
          for (let index = 0; index < number_of_cycle; index++) {
            ...
            reword.userId = user.userId;
            reword.status =
              index == 0 ? RewordStatus.AVAILABLE : RewordStatus.PENDING;
            reword.cardToken = card_token;
            reword.type = type;
            reword.createdDate = new Date();
