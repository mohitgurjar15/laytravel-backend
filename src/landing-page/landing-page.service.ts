...
export class LandingPageService {
    async createNewLandingPage(
        ...
        const { name, templet, default_setting_applied } = createLandingPageDto;
    ...
    async addLandingPageDownPayment(newLandingPageDownPaymentConfigDto: NewLandingPageDownPaymentConfigDto, user: User) {
        ...
        let message = `Down payment configuration added successfully`
        for await (const iterator of module_id) {
            for await (const configId of days_config_id) {
                ...
                if (!config) {
                    ...
                    message = `Down payment configuration updated successfully`
                } else {
