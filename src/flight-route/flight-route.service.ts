...
export class FlightRouteService {
    ...
    async listUnblackListedAirportRoutes(listAirportRouteDto: ListAirportRouteDto) {
        try {
            var {
                limit,
                page_no,
            ...
            let where = `("airport"."is_deleted" = false AND "airport"."is_blacklisted" = false)`;
            // let results = await getConnection()
            //     .createQueryBuilder(Airport, "airport")
            //     .where(where)
            //     .orderBy(`airport.id`, "ASC")
    ...
    async getunBlackListedFlightCountry() {
        let results = await getManager()
            ...
            .where("airports.is_blacklisted = false" )
            .distinctOn(["airports.country"])
            .getMany()
