...
export class CronJobsService {
        ...
        if (cardToken) {
          ...
          if (transaction.status == false) {
            ...
            let param: any = {
              ...
              amount:
                cartBooking.bookings[0].bookingInstalments[0].currency.symbol +
            ...
            for await (const booking of cartBooking.bookings) {
              const nextInstalmentDate = await getManager()
                ...
                .getOne();
              let update = {
                nextInstalmentDate: nextInstalmentDate?.instalmentDate || null,
              };
              if (!nextInstalmentDate) {
            ...
            let param = {
              ...
              cardHolderName:
                transaction.meta_data.transaction.payment_method.full_name,
    ...
    if (
      ...
      if (bookingType == "oneway") {
        ...
        let dto = {
          ...
          flight_class:
            bookingData.moduleInfo[0].routes[0].stops[0].cabin_class,
  ...
  async getDailyPriceOfVacationRental(bookingData: Booking, Headers) {
    let vacationData;
    if (
      new Date(await this.getDataTimefromString(bookingData.checkInDate)) >
      new Date()
    ) {
      Headers["currency"] = bookingData.currency2.code;
      Headers["language"] = "en";
      ...
      vacationData = await new Promise((resolve) =>
        resolve(
          monaker.verifyUnitTypeAvailability(dto, bookingData.user, false)
        )
      );
  ...
  async updateModuleInfo(Headers) {
    ...
    let query = getConnection()
      .createQueryBuilder(Booking, "booking")
      .where(`"booking"."module_id"= 1 AND "booking"."booking_status"= 4`);
