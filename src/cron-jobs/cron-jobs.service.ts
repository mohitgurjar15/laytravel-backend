...
export class CronJobsService {
    ...
    async deltaFlightAvailiblityAssure() {
        const airlines = ["DL"]
        let limit = 20
        let routes = await getConnection()
            .createQueryBuilder(FlightRoute, "routes")
        ...
        for (let index = 0; index < response.length; index++) {
            ...
            emailData[index]['filter_airlines'] = airlines

            const flight = response[index];

            let flightData 

            for await (const iterator of flight?.items) {

                if (airlines.includes(iterator.airline))
                {
                    flightData = iterator
                    break;
                }
